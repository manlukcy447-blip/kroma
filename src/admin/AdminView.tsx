...
export const AdminView:React.FC=()=>{
 ...
 const login=async(e:React.FormEvent)=>{e.preventDefault();setError('');try{const res=await apiFetch<any>('/api/admin/login',{method:'POST',body:JSON.stringify({email,password})});if(res?.token){sessionStorage.setItem('kroma_admin_token',res.token);}sessionStorage.setItem('kroma_admin_session','1');setLoggedIn(true);setPassword('')}catch(e:any){setError(e.message)}};
